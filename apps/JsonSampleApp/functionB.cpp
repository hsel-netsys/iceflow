...
public:
    ...
    [[noreturn]] void
    compute(TSQueue<IceFlowBlock> *input, TSQueue<IceFlowBlock> *output, int output_threshold) {
...
[[noreturn]] void fusion(vector<TSQueue<IceFlowBlock>*> * inputs, TSQueue<IceFlowBlock> * totalInput) {
    while (!inputs->empty()) {
...
void DataFlow(string &sub_syncPrefix,
              int sub,
              string &sub_Prefix_data_main,
              string &sub_Prefix_ack,
              int input_threshold,
              string &pub_syncPrefix,
              string &userPrefix_data_main,
              const string &userPrefix_data_manifest,
              const string &userPrefix_ack,
              int nDataStreams,
              int publish_interval,
              int publish_interval_new,
              int names_in_manifest,
              ...
              int map_threshold)
{
    ...
    //Data
    auto * simple_producer = new ProducerTLV(pub_syncPrefix
            , userPrefix_data_main
            , userPrefix_data_manifest
            , userPrefix_ack
            , nDataStreams
            , publish_interval
            , publish_interval_new
            ,map_threshold);


    auto *simple_consumer = new ConsumerTLV(sub_syncPrefix,
                                            sub_Prefix_data_main,
                                            sub_Prefix_ack,
                                            sub, input_threshold);
    ...
    std::thread th3(&Compute::compute, compute,&totalInput, &simple_producer->output_queue_block, output_threshold);
...
// ----------------------- Consumer------------------------------------------
    ...
    auto sub_Prefix_data_main     = config["Consumer"]["sub_Prefix_data_main"].as<string>();
    auto sub_Prefix_data_manifest = config["Consumer"]["sub_Prefix_data_manifest"].as<string>();
...
// ----------------------- Producer -----------------------------------------
    ...
    auto userPrefix_data_main    = config["Producer"]["userPrefix_data_main"].as<string>();
    auto userPrefix_data_manifest= config["Producer"]["userPrefix_data_manifest"].as<string>();
    ...
    int publish_interval_new            = config["Producer"]["publish_interval_new"].as<int>();
